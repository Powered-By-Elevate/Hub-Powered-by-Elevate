...
interface Props {
  employees: Employee[];
  onClose: () => void;
  onCreated: (applicant: Employee | null) => void;
}

export function AddApplicantModal({ employees, onClose, onCreated }: Props) {
  const [form, setForm] = useState({
    name: '',
    email: '',
    phone: '',
    position_applied_for: '',
    applicant_phase: DEFAULT_APPLICANT_PHASE as ApplicantPhase,
    applicant_stage: DEFAULT_APPLICANT_STAGE,
    hiring_manager_id: '',
    applicant_source: '',
    resume_url: '',
  });
  ...
  const activeEmployees = employees.filter(e => !e.archived && e.lifecycle_status !== 'applicant');
    ...
    >
      ...
      <div className="form-grid">
        <div className="field full">
          ...
          <input type="text" value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))} placeholder="Jane Smith" autoFocus />
        </div>
        <div className="field full">
          <label>Email <span style={{ color: '#E53E3E' }}>*</span></label>
          <input type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email: e.target.value }))} placeholder="jane.smith@email.com" />
        </div>
        <div className="field">
          <label>Phone</label>
        ...
        <div className="field">
          ...
          <input type="text" value={form.position_applied_for} onChange={e => setForm(f => ({ ...f, position_applied_for: e.target.value }))} placeholder="e.g. Senior Project Manager" />
        </div>
        <div className="field">
          <label>Phase</label>
        ...
        <div className="field full">
          ...
          <input type="url" value={form.resume_url} onChange={e => setForm(f => ({ ...f, resume_url: e.target.value }))} placeholder="https://drive.google.com/... or LinkedIn URL" />
        </div>
      </div>
    </Modal>
  );
